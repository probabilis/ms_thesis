...
def different_image_lambdas_and_gammas(exp_data_params, ngd_sim_params, SIMULATE_OR_READ):
    ...
    u0 = initialize_u0_random(exp_data_params.N, REAL=True)
    ...
    _lambda_ls = [0.001, 0.01, 0.1] # ToDo: include in params.json file
    ...
    for kk, _lambda in enumerate(_lambda_ls):
        ...
        for ii, _gamma in enumerate(gamma_ls): 
            ...
            if SIMULATE_OR_READ == "simulate":
                ...
                energies = df_energies["E_total"].values
            ...
            axs[ii, 2*kk + 1].loglog(np.arange(0,len(energies), 1), energies)
