...
if __name__ == "__main__":
    ...
    u_exp = read_csv(INPUT_PATH / f"{dataset}/csv/mcd_slice_{recording}.csv", OUTPUT_PATH / "raw.png", PLOT = True)
    ...
    fig, axs = plt.subplots( len(gamma_ls), 2 * len(_lambda_ls), figsize = (len(gamma_ls) * 4, len(_lambda_ls) * 4 ) )
    ...
    for ii, gamma in enumerate(gamma_ls):
        for jj, _lambda in enumerate(_lambda_ls):
            ...
            axs[ii, 2*jj].imshow(torch.where(torch.abs(u_sim) < THRESHOLD, 2.0, u_sim), cmap = main_colormap, origin="lower", extent=(0,1,0,1))
            axs[ii, 2*jj].set_box_aspect(1)
            axs[ii, 2*jj].axes.get_xaxis().set_ticks([])
            axs[ii, 2*jj].axes.get_yaxis().set_ticks([])
            axs[ii, 2*jj].set_title(f"$P(u) = {perimeter:.2f} / S(u) = {fisherJ:.3f}$", fontsize = 8)
