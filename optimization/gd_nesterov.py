...
def gradient_descent_nesterov(u0, LIVE_PLOT, DATA_LOG, FOLDER_PATH, gridsize, N, th, gamma, epsilon, tau, c0, num_iters, prox_newton_iters, tol_newton, LAPLACE_SPECTRAL = False, STOP_BY_TOL = True, ENERGY_STOP_TOL = 1e-10, PBC = True):
    """
    Nesterov Proximal Gradient Descent algo of energy functional
    """ 
    ...
    try:
        for ii in tqdm(range(1, num_iters+1), desc="Nesterov GD"):
            # 1) extrapolation
            ...
            # 5) energy evaluation
            if LAPLACE_SPECTRAL:
                E_TOTAL = energy_value(gamma, epsilon, N, u_curr, c0, sigma_k, modk2)
            else:
                E_TOTAL = energy_value_fd(u_curr, sigma_k, N, gamma, epsilon, c0, PBC)
