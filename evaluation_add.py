...
def different_image_preprocessings(labyrinth_data_params, ngd_sim_params, SIMULATE_OR_READ):
    ...
    for _type in types:
        ...
        _lambda_ls = [float(torch.std(u0)) , 0.1, 0.2]

        print(f"Learning Rate Lambda := std(u0) = {_lambda_ls}")
        print_bars()
        ...
        for kk, _lambda in enumerate(_lambda_ls):
            ...
            for ii, _gamma in enumerate(gamma_ls): 
                ...
                if SIMULATE_OR_READ == "simulate":
                    print("Simulating")
                    u, energies = gradient_descent_nesterov_evaluation(u0, u_exp, _lambda, LIVE_PLOT, DATA_LOG, OUTPUT_PATH, **asdict(labyrinth_data_params),**asdict(ngd_sim_params), STOP_BY_TOL=True, ENERGY_DIFF_STOP_TOL = ENERGY_DIFF_STOP_TOL)
...
if __name__ == "__main__":
    different_image_preprocessings(labyrinth_data_params, ngd_sim_params, "read")
