...
Nmax = 50;                 % Max iterations for fixed point
...
k = [1:N/2 -N/2:-1]';   % Proper wave number vector for FFT
...
%u0 = (2*rand(N,N) - 1) + 1i*(2*rand(N,N) - 1);  % Complex initial condition

u0 = 2 * rand(N,N) - 1;

%u0 = tanh(10 * (rand(N, N) - 0.5)) + 1i * tanh(10 * (rand(N, N) - 0.5));
%[x1, x2] = ndgrid(x, x);
%u0 = sin(8 * pi * x1) * sin(8 * pi * x2);
...
while (n_it < max_it) && (abs(Denergy) > stop_crit);
    ...
    disp(dt);
    if conv == 1
        [Energy(1,i)] = energy_value(gamma, epsilon, N, u, th, modk, modk2, c1);
        ...
        n_it = n_it + 1;
        i = i + 1;
        filename_image = sprintf('condette_image_N=%d_max_it=%d_gamma=%d_eps=%d_dt=%d_th=%d.png',N,max_it,gamma,epsilon,dt,th);
        saveas(fig1, filename_image);
        filename_energy = sprintf('condette_energy_N=%d_max_it=%d_gamma=%d_eps=%d_dt=%d_th=%d.png',N, max_it,gamma,epsilon,dt,th);
